...
public class StudentFetcher extends AbstractFetcher {
    ...
    private String login() {
        ...
        try {
            Connection loginConnection = Jsoup.connect(HAC_LOGIN_URL).userAgent("Mozilla/5.0 (X11; Linux x86_64; rv:67.0) Gecko/20100101 Firefox/67.0");
            Document loginPage = loginConnection.get();
            String requestVerificationToken = loginPage.selectFirst("input[name=__RequestVerificationToken]").val();

            sessionID = loginConnection.response().cookie("ASP.NET_SessionId");

            Response loginResponse = loginConnection.data("Database", "10")
                    .data("__RequestVerificationToken", requestVerificationToken)
                    .data("LogOnDetails.UserName", currentUser.getUsername())
                    .data("LogOnDetails.Password", currentUser.getPassword())
                    .method(Method.POST)
                    .cookies(loginConnection.response().cookies())
